...
const createTransporter = async () => {
  ...
  if (!isConfigured) {
    console.log("⚠️  Email real não configurado. Usando Ethereal (teste).");
...
const templateConvite = ({ nomeEvento, data, local, mensagem, linkConvite, nomeAnfitriao }) => {
  ...
  return `<!DOCTYPE html>
<html lang="pt">
<head><meta charset="UTF-8"/><title>Convite - ${nomeEvento}</title></head>
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.1);">
        <tr>
          <td style="background:linear-gradient(135deg,#667eea,#764ba2);padding:40px;text-align:center;">
            ...
            <p style="color:rgba(255,255,255,0.85);margin:8px 0 0;font-size:15px;">Você recebeu um convite</p>
          </td>
        </tr>
        <tr>
          <td style="padding:40px;">
            <h2 style="color:#333;font-size:24px;margin:0 0 8px;">${nomeEvento}</h2>
            <p style="color:#666;font-size:15px;margin:0 0 30px;">${nomeAnfitriao} convidou-o para este evento especial.</p>
            ${mensagem ? `<div style="background:#f8f9ff;border-left:4px solid #667eea;padding:16px 20px;border-radius:0 8px 8px 0;margin-bottom:30px;">
            ...
            <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:30px;">
              <tr><td style="padding:12px 16px;background:#f8f9ff;border-radius:8px 8px 0 0;border-bottom:1px solid #e8ecff;">
                <span style="color:#667eea;font-weight:700;font-size:13px;text-transform:uppercase;">Data</span><br/>
              ...
              <tr><td style="padding:12px 16px;background:#f8f9ff;border-radius:0 0 8px 8px;">
                <span style="color:#667eea;font-weight:700;font-size:13px;text-transform:uppercase;">Local</span><br/>
                <span style="color:#333;font-size:15px;">${local}</span>
              </td></tr>
            </table>
            <div style="text-align:center;margin:30px 0;">
              <a href="${linkConvite}" style="display:inline-block;background:linear-gradient(135deg,#667eea,#764ba2);color:#fff;text-decoration:none;padding:16px 40px;border-radius:8px;font-size:16px;font-weight:700;">
                Confirmar Presença
              </a>
            </div>
            <p style="color:#999;font-size:13px;text-align:center;margin:0;">
              Ou acesse: <a href="${linkConvite}" style="color:#667eea;">${linkConvite}</a>
            </p>
          </td>
        </tr>
        <tr>
...
const templateConfirmacao = ({ nomeConvidado, nomeEvento, data, local, confirmado, linkConvite }) => {
  ...
  const cor = confirmado ? "#667eea,#764ba2" : "#f5576c,#f093fb";
...
<body style="margin:0;padding:0;background:#f4f4f4;font-family:Arial,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f4f4f4;padding:40px 0;">
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.1);">
        <tr>
          <td style="background:linear-gradient(135deg,${cor});padding:36px;text-align:center;">
        ...
        <tr>
          <td style="padding:36px;">
            ...
            <p style="color:#666;font-size:15px;margin:0 0 24px;">
              ${confirmado
                ? `A sua presença no evento <strong>${nomeEvento}</strong> foi confirmada.`
            ...
            ${confirmado ? `<div style="text-align:center;">
...
const enviarConvite = async ({ emails, evento, linkConvite, nomeAnfitriao }) => {
  ...
  const resultados = await Promise.allSettled(
    emails.map(email =>
      transporter.sendMail({
        ...
        return { email, sucesso: true, ...(previewUrl && { previewUrl }) };
      })
    )
  );
  ...
  return {
    ...
    resultados: resultados.map((r, i) =>
      r.status === "fulfilled"
        ? r.value
        : { email: emails[i], sucesso: false, erro: r.reason?.message }
    ),
  };
};
